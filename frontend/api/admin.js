...
export const getAllQuestionList = async (param, success, fail) => {
  await api.get(`/admin/interview`, {params: param}).then(success).catch(fail);
};

//질문 공개
export const setQuestionOpen = async (interviewId, success, fail) => {
  await api.patch(`/admin/interview/${interviewId}/makePublic`).then(success).catch(fail);
};

//질문 비공개
export const setQuestionPrivate = async (interviewId, success, fail) => {
  await api.patch(`/admin/interview/${interviewId}/hide`).then(success).catch(fail);
};
