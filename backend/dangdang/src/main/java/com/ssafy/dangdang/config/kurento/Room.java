...
public class Room implements Closeable {
    ...
    public UserSession join(String userName, WebSocketSession session) throws IOException {
        ...
        if (userName.equals("screen")) {
            setScreen(participant);  // 기존에 방에 있던 참가자들에게 'newParticipantArrived'라는 메세지를 보내줌
            sendParticipantNamesForScreen(participant); // Screen을 보내줄 수 있도록 existingParticipantsForScreen 메세지 보내줌
            participants.put(participant.getName(), participant); // Room의 참가자 관리 목록에 Screen 추가
            return participant;
        }
    ...
    private Collection<String> setScreen(UserSession screen) throws IOException {
        final JsonObject newScreenMsg = new JsonObject();
        newScreenMsg.addProperty("id", "newScreenArrived");
        newScreenMsg.addProperty("name", screen.getName());
        
        final List<String> participantsList = new ArrayList<>(participants.values().size());
        log.debug("ROOM {}: notifying other participants of new Screen {}", name, screen.getName());
    ...
    public void sendParticipantNamesForScreen(UserSession user) throws IOException {
        ...
        existingParticipantsMsg.addProperty("id", "existingParticipantsForScreen");
